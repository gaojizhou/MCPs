...
TOOLS: list[dict[str, Any]] = [
    ...
    {"name": "find_person", "description": "Preferred tool whenever the user asks to find a person. It first enables native SmartTrack and disables the recognition-object rectangle. YOLO checks the view and moves the PTZ search plan only while no person is visible. As soon as YOLO finds a person, manual PTZ corrections stop and the tool waits for native SmartTrack to centre them. The exact final high-resolution image is YOLO-verified as centred before it is returned.", "inputSchema": {"type": "object", "properties": {"moves": {"type": "array", "items": {"type": "string", "enum": ["up", "down", "left", "right"]}, "maxItems": 24, "default": ["left", "left", "left", "left"]}, "speed": {"type": "integer", "minimum": 1, "maximum": 10, "default": 3}, "steps_per_move": {"type": "integer", "minimum": 1, "maximum": 30, "default": 4}, "confidence": {"type": "number", "minimum": 0.1, "maximum": 0.95, "default": 0.45}, "center_tolerance": {"type": "number", "minimum": 0.03, "maximum": 0.3, "default": 0.06}}}},
        ...
        for index, position in enumerate(positions):
            ...
            if matches:
                ...
                if native_tracking:
                    ...
                    summary = {"found": True, "position": position, "object": target, "centered": True, "centering_method": "camera_native_smarttrack", "native_tracking_observations": observations, "manual_centering_adjustments": 0, "camera_locked_on_position": True, "note": "YOLO found the person; native SmartTrack centred them and the returned main-stream frame was verified."}
